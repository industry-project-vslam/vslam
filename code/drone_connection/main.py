...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--fullscreen", action="store_true", help="Open the main camera preview fullscreen")
    parser.add_argument("--debug-stream", action="store_true", help="Print low-level AI-deck stream diagnostics")
    parser.add_argument("--no-analysis", action="store_true", help="Disable real-time human posture detection overlay")
    parser.add_argument("--detector-model", default=None, help="YOLO human detector .pt path")
    parser.add_argument("--classifier-model", default=None, help="MobileNet posture classifier .pt path")
    parser.add_argument("--class-map", default=None, help="Posture classifier class_to_idx.json path")
    parser.add_argument("--det-conf", type=float, default=0.23, help="Human detector confidence threshold")
    parser.add_argument("--posture-conf", type=float, default=0.50, help="Mark posture unknown below this top score")
    parser.add_argument("--posture-margin", type=float, default=0.0, help=argparse.SUPPRESS)
    parser.add_argument("--analysis-stride", type=int, default=2, help="Run analysis every N displayed frames")
...
def run_cli(args: argparse.Namespace) -> None:
    ...
    if not args.no_analysis:
        from src.posture_pipeline import PosturePipeline, PosturePipelineSettings

        pipeline_settings = PosturePipelineSettings(
            detector_confidence=args.det_conf,
            posture_confidence=args.posture_conf,
            analyze_every_n_frames=args.analysis_stride,
        )
        if args.detector_model:
            pipeline_settings.detector_model = Path(args.detector_model)
        if args.classifier_model:
            pipeline_settings.classifier_model = Path(args.classifier_model)
        if args.class_map:
            pipeline_settings.class_map = Path(args.class_map)
        frame_processor = PosturePipeline(pipeline_settings).process
    ...
    settings = StreamSettings(
        ...
        display_fullscreen=args.fullscreen,
...
def main() -> None:
    ...
    if args.cli:
        ...
        launch_gui(
            ...
            analyze_frames=not args.no_analysis,
