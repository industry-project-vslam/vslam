...
class SourceRequest(PipelineParams):
    ...
    drone_id: str
    source_path: str
    map_id: str | None = None  # If specified, this drone will be registered to the same map
...
def _run_pipeline(
    ...
    return _build_response(drone_id, tracker_service)
...
def process_source(
    ...
    If map_id is specified, this drone will be registered to that map so its
    point cloud will be merged with other drones in the same map.
    """
    params = {
        k: v
    ...
    return _run_pipeline(request.drone_id, request.source_path, params, tracker_service, map_id=request.map_id)
...
async def upload_frames(
    ...
    map_id: str | None = None,  # Optional: if specified, register to this map
    ...
    If map_id is specified, this drone's point cloud will be merged with other
    drones registered to the same map.
    """
    if not files:
