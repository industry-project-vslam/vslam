...
class AIDeckApp(ttk.Frame):
    def __init__(
        ...
        self.display_fullscreen = tk.BooleanVar(value=display_fullscreen)
        self.debug_stream = tk.BooleanVar(value=debug_stream)
        self.analyze_frames = tk.BooleanVar(value=analyze_frames)
        self.detector_model = tk.StringVar(value=detector_model or str(default_detector_model()))
        self.classifier_model = tk.StringVar(value=classifier_model or str(default_classifier_model()))
        self.class_map = tk.StringVar(value=class_map or str(default_class_map()))
        self.detector_confidence = tk.DoubleVar(value=detector_confidence)
        self.posture_confidence = tk.DoubleVar(value=posture_confidence)
        self.analyze_every_n_frames = tk.IntVar(value=analyze_every_n_frames)
    ...
    def _build(self) -> None:
        ...
        ttk.Checkbutton(self, text="Fullscreen camera window", variable=self.display_fullscreen).grid(
        ...
        ttk.Checkbutton(self, text="Show real-time human posture detections", variable=self.analyze_frames).grid(
            row=9, column=0, columnspan=2, sticky="w", pady=4
        )

        analysis = ttk.LabelFrame(self, text="Posture analysis")
        analysis.grid(row=10, column=0, columnspan=2, sticky="ew", pady=(8, 4))
        analysis.columnconfigure(1, weight=1)

        ttk.Label(analysis, text="Analyze every N frames").grid(row=0, column=0, sticky="w", pady=2, padx=(8, 6))
        ttk.Spinbox(analysis, from_=1, to=10, textvariable=self.analyze_every_n_frames, increment=1).grid(
            row=0, column=1, sticky="ew", pady=2, padx=(0, 8)
        )
        ttk.Label(analysis, text="Detector confidence").grid(row=1, column=0, sticky="w", pady=2, padx=(8, 6))
        ttk.Spinbox(analysis, from_=0.05, to=0.95, textvariable=self.detector_confidence, increment=0.05).grid(
            row=1, column=1, sticky="ew", pady=2, padx=(0, 8)
        )
        ttk.Label(analysis, text="Unknown below confidence").grid(row=2, column=0, sticky="w", pady=2, padx=(8, 6))
        ttk.Spinbox(analysis, from_=0.05, to=0.95, textvariable=self.posture_confidence, increment=0.05).grid(
            row=2, column=1, sticky="ew", pady=2, padx=(0, 8)
        )

        buttons = ttk.Frame(self)
        buttons.grid(row=11, column=0, columnspan=2, sticky="ew", pady=10)
        ...
        ttk.Separator(self).grid(row=12, column=0, columnspan=2, sticky="ew", pady=10)
        ttk.Label(self, textvariable=self.status).grid(row=13, column=0, columnspan=2, sticky="w")
        ttk.Label(self, textvariable=self.frame_status).grid(row=14, column=0, columnspan=2, sticky="w", pady=(4, 0))
        ...
        ttk.Label(self, text=note, wraplength=650).grid(row=15, column=0, columnspan=2, sticky="w", pady=(16, 0))
    ...
    def start_stream(self) -> None:
        ...
        try:
            frame_processor = self._create_frame_processor()
        except Exception as exc:
            self.status.set(f"Could not load posture analysis: {exc}")
            return
        ...
        settings = StreamSettings(
            ...
            display_fullscreen=bool(self.display_fullscreen.get()),
    ...
    def _create_frame_processor(self):
        if not self.analyze_frames.get():
            return None

        from .posture_pipeline import PosturePipeline, PosturePipelineSettings

        self.status.set("Loading posture models...")
        settings = PosturePipelineSettings(
            detector_model=Path(self.detector_model.get()),
            classifier_model=Path(self.classifier_model.get()),
            class_map=Path(self.class_map.get()),
            detector_confidence=float(self.detector_confidence.get()),
            posture_confidence=float(self.posture_confidence.get()),
            analyze_every_n_frames=int(self.analyze_every_n_frames.get()),
        )
        pipeline = PosturePipeline(settings)
