...
static unsigned char *streamBuff;
...
void createImageHeaderPacket(CPXPacket_t * packet, uint32_t imgSize, StreamerMode_t imgType) {
  ...
  imgHeader->width = STREAM_WIDTH;
  imgHeader->height = STREAM_HEIGHT;
  imgHeader->depth = 1;
...
void camera_task(void *parameters)
{
  ...
  if (streamBuff == NULL)
  {
    cpxPrintToConsole(LOG_TO_CRTP, "Failed to allocate Memory for Stream Image \n");
    return;
  }
  ...
  enc_conf.width = STREAM_WIDTH;
  enc_conf.height = STREAM_HEIGHT;
  ...
  pi_buffer_init(&buffer, PI_BUFFER_TYPE_L2, streamBuff);
  ...
  // This must fit the full encoded JPEG. Keep margin for complex/noisy scenes.
  jpeg_data.size = 1024 * 30;
  ...
  while (1)
  {
    if (wifiClientConnected == 1)
    {
      ...
      if (frameTime < targetFrameTicks)
      {
        vTaskDelay(targetFrameTicks - frameTime);
      }
    }
    else
    {
