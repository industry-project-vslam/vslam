...
- [FastAPI Server & Multi-Drone Mapping](#fastapi-server--multi-drone-mapping)
  - [Starting the API Server](#starting-the-api-server)
  - [Multi-Drone Same-Map Stitching](#multi-drone-same-map-stitching)
  - [Key Endpoints](#key-endpoints)
  - [Degradation Handling](#degradation-handling)
...
## FastAPI Server & Multi-Drone Mapping

A FastAPI server is provided for programmatic access to the SLAM3R reconstruction pipeline, supporting multi-drone scenarios with coordinate frame alignment.

### Starting the API Server

```bash
python app_api_gradio.py
```

The server runs on `http://localhost:8000` by default. Interactive API documentation is available at `http://localhost:8000/docs` (Swagger UI).

### Multi-Drone Same-Map Stitching

Multiple drones can be registered to the same coordinate frame by specifying a `map_id`. When multiple drones process frames with the same `map_id`, their point clouds are automatically merged into a single coordinate frame.

**Example: Process two drones and stitch them to the same map**

```bash
# Drone 1 uploads frames to map "my_map"
curl -X POST "http://localhost:8000/api/upload_frames/drone_1?map_id=my_map" \
  -F "files=@frame_001.jpg" \
  -F "files=@frame_002.jpg"

# Drone 2 uploads frames to the same map
curl -X POST "http://localhost:8000/api/upload_frames/drone_2?map_id=my_map" \
  -F "files=@frame_001.jpg" \
  -F "files=@frame_002.jpg"

# Result: Merged point cloud saved to results_online/maps/my_map/merged.ply
```

### Key Endpoints

- **POST /api/upload_frames/{drone_id}** — Upload frame images and run reconstruction
  - Query parameters: `map_id` (optional) — Register drone to this coordinate frame
  - Returns: Trajectory, point count, save directory

- **GET /api/pointcloud/{drone_id}** — Download point cloud (PLY or NPY format)
  - Query parameters: `format` (default: `ply`)

- **GET /api/pointcloud/{drone_id}/trajectories** — Get camera trajectory

### Degradation Handling

The system gracefully handles frames with low confidence:
- Single bad frames are skipped without triggering reconstruction failures
- Reconstruction continues automatically after frame drops
- Loop closure detection helps recover trajectory consistency across degraded segments
