...
class WallFollowing():
    ...
    def wall_follower(self, front_range, side_range, current_heading,
        ...
        # Emergency proximity check: if side or front sensor reports a distance
        # smaller than emergency_distance, immediately back away from the obstacle.
        try:
            if side_range is not None and float(side_range) < float(self.emergency_distance):
                # Move away from wall: velocity_y should push away from wall
                velocity_x = 0.0
                ...
                return velocity_x, velocity_y, rate_yaw, self.state
        except Exception:
            pass
        try:
            if front_range is not None and float(front_range) < float(self.emergency_distance):
                # Back up a bit
                velocity_x = -self.max_forward_speed / 2.0
                velocity_y = 0.0
                rate_yaw = 0.0
                self.state = self.state_transition(self.StateWallFollowing.HOVER)
                return velocity_x, velocity_y, rate_yaw, self.state
