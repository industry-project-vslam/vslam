...
class FrontierMap:
    def __init__(self, size=MAP_SIZE, cell_size=CELL_SIZE, world_scale=WORLD_SCALE,
                 allow_obstacle_reclassification=True):
        self.size = size
        self.cell_size = cell_size
        self.world_scale = world_scale
        self.grid = np.zeros((size, size), dtype=np.uint8)
        self.last_pose = (size // 2, size // 2, 0.0)
        self.allow_obstacle_reclassification = allow_obstacle_reclassification
    ...
    def update_from_pose_and_ranges(self, pose, meas):
        ...
        for k, dist_mm in ranges.items():
            ...
            # Clamp measured distance to max range
            clamped_dist = min(dist_m, MAX_RANGE_M)

            ex = x + math.cos(ang) * clamped_dist
            ey = y + math.sin(ang) * clamped_dist
            ...
            if len(line) > 1:
                # Walk along the ray from drone to measurement endpoint
                for i, (px, py) in line[:-1]:
                    if not self.in_bounds(px, py):
                        continue

                    # Distance from drone to this cell in world units
                    pgx, pgy = self.grid_to_world(px, py)
                    d_cell = math.sqrt((pgx - x) ** 2 + (pgy - y) ** 2)

                    # If this cell was an obstacle but the sensor sees beyond it,
                    # the obstacle is likely moved → mark as FREE
                    if self.allow_obstacle_reclassification and self.grid[py, px] == OBSTACLE and dist_m > d_cell:
                        self.grid[py, px] = FREE

                    # Otherwise, mark as FREE if we haven't marked it as obstacle yet
                    if self.grid[py, px] != OBSTACLE:
                        self.grid[py, px] = FREE

                # Endpoint: if the measured distance is small and within obstacle range,
                # mark as OBSTACLE
    ...
    def update_frontiers(self):
        frontier = np.zeros_like(self.grid, dtype=bool)
        ...
        # Only set FRONTIER on cells that are FREE
        self.grid[(self.grid == FREE) & frontier] = FRONTIER
